		...
		try {
			...
			if (!localChecker.some((el) => el.valid === false)) {
				...
				if (newData.status) {
					...
					setIsUpdating(false);
					dispatch(
						showError(newData?.data?.detail || "Could not add new permit")
					);
				}
			} else {
			...
			dispatch(
				showError(err?.response?.data?.title || "Could not add new permit")
			);
		}
	};
...
export default AddOrEditPermit;
