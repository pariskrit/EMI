...
import FeedbackPrioritiesTable from "./FeedbackPrioritiesTable";
...
const FeedbackPrioritiesContent = ({ navigation, id, setIs404, state }) => {
	// Init state
	...
	const handleGetData = useCallback(async () => {
		...
		try {
			...
			let result = await API.get(
				`/api/ApplicationFeedbackPriorities?applicationId=${id}`
			);
	...
	const handleDefaultUpdate = async () => {
		...
		try {
			...
			const result = await API.patch(`/api/Applications/${id}`, [
				{
					op: "replace",
					path: "defaultFeedbackPriorityID",
	...
	const handleSearch = () => {
		...
		// Creating regex
		let check = new RegExp(searchQuery, "gi");
		...
		const getApplicationData = async () => {
			...
			try {
				...
				if (result.status === 200) {
					...
					// Setting default
					setDefaultData(result.data.defaultFeedbackPriorityID);
				} else {
					// If error, throwing to catch
					throw new Error(result);
				}
	...
	return (
		<div>
			...
			<DefaultDialog
				...
				data={confirmDefault}
				entity="Feedback Priority"
				handleDefaultUpdate={handleDefaultUpdate}
			/>
			<DeleteDialog
				entityName="Feedback Priority"
				open={openDeleteDialog}
				closeHandler={handleDeleteDialogClose}
				deleteEndpoint="/api/ApplicationFeedbackPriorities"
				deleteID={deleteID}
...
export default FeedbackPrioritiesContent;
