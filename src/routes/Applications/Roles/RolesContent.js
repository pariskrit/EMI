...
const RolesContent = ({ navigation, id, setIs404, state }) => {
	...
	const handleGetData = useCallback(async () => {
		...
		// Attempting to get elements
		try {
			// Getting data from API
			let result = await API.get(`/api/ApplicationRoles?applicationId=${id}`);
	...
	// Fetch Side effect to get elements
	useEffect(() => {
		// Getting elements and updating state
		handleGetData()
			.then(() => {
				// Rendering elements
				setHaveData(true);
	...
	return (
		<div>
			...
			<DeleteDialog
				...
				deleteEndpoint="/api/ApplicationRoles"
				deleteID={deleteID}
