...
const ServiceStatusPage = () => {
	...
	const initialCrumb = [{ id: 1, name: "Status Reason" }];
	const [chartData, setChartData] = useState([]);
	const [barStatusReason, setBarStatusReason] = useState({});
	...
	const fetchPlannedWorkCompliance = useCallback(
		async ({
			...
			const response = await getServiceStatusChangesByReason({
				startDate,
				endDate,
				siteDepartmentId,
				modelId,
	...
	const handleBarClick = async (data, index, e) => {
		...
		if (chartData.depth === "R") {
			response = await getServiceStatusChangesByReason({
				siteDepartmentId: selectedDepartment?.id,
				modelId: selectedModel.id,
				startDate: selectedTimeframe.fromDate,
				endDate: selectedTimeframe.toDate,
				changeStatusReasonId: data?.id,
			});
			if (response?.status) {
				setBarStatusReason(data);
				setChartData(response?.data);
				setCrumbs([
		...
		if (chartData.depth === "M") {
			response = await getServiceStatusChangesByReason({
				...
				startDate: selectedTimeframe.fromDate,
				endDate: selectedTimeframe.toDate,
				changeStatusReasonId: barStatusReason.id,
			});
			if (response?.status) {
				setChartData(response?.data);
				setLast(true);
				...
				setCrumbs([
					{ id: 2, name: barStatusReason.name, depth: "R" },
	...
	const handleRevertChart = async (data) => {
		...
		if (data.depth === "M") {
			...
			revertCrumbs = [
				{ id: 1, name: barStatusReason.name, depth: "R" },
		...
		<div className={"container"}>
			<NavDetails
				...
				staticCrumbs={[
					...
					{
						id: 2,
						name: `${customCaptions.service} Status Change Reasons`,
					},
				]}
			/>
			<CustomDateRange
			...
			<GraphAnalyticsTitle
				datas={crumbs}
				title={`${customCaptions.service} Status Change Reasons`}
				subTitle={subTitle}
