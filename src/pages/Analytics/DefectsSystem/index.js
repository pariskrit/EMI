...
const DefectsSystemPage = () => {
	...
	const fetchDefectsBySystem = useCallback(
		async ({
			siteDepartmentID = "",
			modelId = "",
			startDate = "",
			...
			setIsloading(true);
			const response = await getDefectsBySystem({
				startDate,
				endDate,
				defectRiskRatingId,
	...
	useEffect(() => {
		fetchDefectsBySystem({ siteDepartmentID });
		if (siteDepartmentID) {
			setShowData({
				...showData,
	...
	const handleBarClick = async (data) => {
		...
		if (chartData.depth === "S") {
			...
			response = await getDefectsBySystem({
				siteDepartmentId: selectedDepartment?.id,
				defectRiskRatingId: selectedRating.id,
				modelId: selectedModel.id,
				startDate: selectedTimeframe.fromDate,
...
export default DefectsSystemPage;
