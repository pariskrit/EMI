...
const ServiceStopPage = () => {
	...
	const initialCrumb = [{ id: 1, name: customCaptions.stopReason }];
	const [chartData, setChartData] = useState([]);
	const [barStopReason, setBarStopReason] = useState({});
	...
	const fetchPlannedWorkCompliance = useCallback(
		async ({
			...
			const response = await getStopsByReason({
				startDate,
				endDate,
				siteDepartmentId,
				modelId,
	...
	const handleBarClick = async (data, index, e) => {
		...
		if (chartData.depth === "R") {
			response = await getStopsByReason({
				siteDepartmentId: selectedDepartment?.id,
				modelId: selectedModel.id,
				startDate: selectedTimeframe.fromDate,
				endDate: selectedTimeframe.toDate,
				stopReasonId: data?.id,
			});
			if (response?.status) {
				setBarStopReason(data);
				setChartData(response?.data);
		...
		if (chartData.depth === "M") {
			response = await getStopsByReason({
				...
				startDate: selectedTimeframe.fromDate,
				endDate: selectedTimeframe.toDate,
				stopReasonId: barStopReason.id,
			});
			if (response?.status) {
				setChartData(response?.data);
				setLast(true);
				setCrumbs([
					{ id: 2, name: barStopReason.name, depth: "R" },
	...
	const handleRevertChart = async (data) => {
		...
		if (data.depth === "M") {
			...
			revertCrumbs = [
				{ id: 1, name: barStopReason.name, depth: "R" },
		...
		<div className={"container"}>
			<NavDetails
				...
				staticCrumbs={[
					...
					{
						id: 2,
						name: `${customCaptions?.service} Stop Reasons`,
					},
				]}
			/>
			<CustomDateRange
