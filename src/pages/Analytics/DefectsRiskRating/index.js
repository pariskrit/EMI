...
const DefectsRiskRatingPage = () => {
	...
	const onDropdownChange = async (type, selectedItem) => {
		if (type === "department" && selectedItem.id !== selectedDepartment.id) {
			...
			await fetchDefectsByRiskRating({
	...
	const fetchDefectsByRiskRating = useCallback(
		async ({
			siteDepartmentID = "",
			modelId = "",
			startDate = "",
			...
			setIsloading(true);
			const response = await getDefectsByRiskRating({
				startDate,
				endDate,
	...
	useEffect(() => {
		fetchDefectsByRiskRating({ siteDepartmentID });
		if (siteDepartmentID) {
			setShowData({
				...showData,
	...
	const handleBarClick = async (data) => {
		...
		if (chartData.depth === "R") {
			response = await getDefectsByRiskRating({
				siteDepartmentId: selectedDepartment?.id,
				modelId: selectedModel.id,
				startDate: selectedTimeframe.fromDate,
				endDate: selectedTimeframe.toDate,
				defectRiskRatingId: data.id,
			});
			setShowData({
				...showData,
				riskRatings: {
...
export default DefectsRiskRatingPage;
