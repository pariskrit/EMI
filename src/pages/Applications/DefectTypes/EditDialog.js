...
const EditDefectTypeDialog = ({ open, closeHandler, data, handleEditData }) => {
	// Init state
	...
	const handleUpdateData = async () => {
		...
		try {
			let updateName = await API.patch(
				`/api/ApplicationDefectTypes/${data.id}`,
				[
					{
						op: "replace",
						path: "name",
	...
	const handleSave = async () => {
		...
		try {
			...
			setIsUpdating(false);
			closeOverride();
			dispatch(showError("Failed to edit defect type."));
		}
	};
			...
			>
				...
				<AED.ActionContainer>
					<DialogTitle id="alert-dialog-title">
						{<AED.HeaderText>Edit Defect Type</AED.HeaderText>}
					</DialogTitle>
					<AED.ButtonContainer>
						<AED.CancelButton
...
export default EditDefectTypeDialog;
