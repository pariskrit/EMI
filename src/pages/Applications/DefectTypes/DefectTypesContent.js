...
import DefectTypesTable from "./DefectTypesTable";
...
import { getApplicationDefectTypes } from "services/History/application";

// Init styled components
const AC = ContentStyle();

const DefectTypesContent = ({ navigation, id, setIs404, state }) => {
	// Init state
	...
	const handleGetData = useCallback(async () => {
		...
		try {
			...
			let result = await API.get(
				`/api/ApplicationDefectTypes?applicationId=${id}`
			);
	...
	useEffect(() => {
		...
		handleGetData()
			...
			.catch((err) =>
				dispatch(showError("Failed to fetch defect risk rating."))
			);
	}, [handleGetData]);
	...
	return (
		<div className="container">
			<TabTitle title={`${applicationName} Defect Types`} />
			{/* Start dialogs */}
			<HistoryBar
				id={id}
				...
				fetchdata={(id, pageNumber, pageSize) =>
					getApplicationDefectTypes(id, pageNumber, pageSize)
				}
			/>
			<AddDialog
				open={openAddDialog}
			...
			/>
			<DeleteDialog
				entityName="Defect Type"
				open={openDeleteDialog}
				closeHandler={handleDeleteDialogClose}
				deleteEndpoint="/api/ApplicationDefectTypes"
				deleteID={deleteID}
...
export default DefectTypesContent;
