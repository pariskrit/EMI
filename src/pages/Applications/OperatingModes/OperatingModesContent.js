...
import { getApplicationOperaingModes } from "services/History/application";
// Init styled components
const AC = ContentStyle();

const OperatingModesContent = ({ navigation, id, setIs404, state }) => {
	// Init state
	...
	const handleGetData = useCallback(async () => {
		...
		try {
			...
			let result = await API.get(
				`/api/ApplicationOperatingModes?applicationId=${id}`
			);
			...
			dispatch(showError("Failed to fetch operating modes."));
			return false;
		}
	}, [id, setIs404]);
	...
	const handleDefaultUpdate = async () => {
		...
		try {
			...
			// TODO: real error handling
			dispatch(showError("Failed to update default operating modes."));
			return false;
		}
	};
		...
		handleGetData()
			.then((data) => {
				if (data) {
					...
					throw new Error("Failed to fetch operating modes.");
				}
			})
			.catch((err) => dispatch(showError("Failed to fetch operating modes.")));
	}, [handleGetData]);
		...
		const getApplicationData = async () => {
			...
			try {
				...
				if (result.status === 200) {
					...
					// Setting default
					setDefaultData(result.data.defaultOperatingModeID);
				} else {
					// If error, throwing to catch
					throw new Error(result);
				}
	...
	return (
		<div className="container">
			<TabTitle title={`${applicationName} Operating Modes`} />
			{/* START DIALOGS */}
			<HistoryBar
				id={id}
				...
				fetchdata={(id, pageNumber, pageSize) =>
					getApplicationOperaingModes(id, pageNumber, pageSize)
				}
			/>
			<AddDialog
				open={openAddDialog}
			...
			<DeleteDialog
				...
				deleteEndpoint="/api/ApplicationOperatingModes"
				deleteID={deleteID}
...
export default OperatingModesContent;
