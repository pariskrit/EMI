...
const EditStatusChangeDialog = ({
	...
	const handleUpdateData = async () => {
		...
		try {
			let updateName = await API.patch(
				`/api/ApplicationChangeStatusReasons/${data.id}`,
				[
					{
						op: "replace",
						path: "name",
	...
	const handleSave = async () => {
		...
		try {
			...
			setIsUpdating(false);
			closeOverride();
			dispatch(showError("Failed to edit status change."));
		}
	};
	...
	// Updating data after inital load
	useEffect(() => {
		if (data !== null && open) {
			...
			>
				...
				<AED.ActionContainer>
					<DialogTitle id="alert-dialog-title">
						{<AED.HeaderText>Edit Status Change</AED.HeaderText>}
					</DialogTitle>
					<AED.ButtonContainer>
						<AED.CancelButton
...
export default EditStatusChangeDialog;
