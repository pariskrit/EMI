...
import { getApplicationSkipReason } from "services/History/application";
...
const SkippedTasksContent = ({ navigation, id, setIs404, state }) => {
	...
	const handleGetData = useCallback(async () => {
		...
		try {
			...
			dispatch(showError("Failed to fetch skipped tasks."));
			return false;
		}
	}, [id, setIs404]);
	...
	useEffect(() => {
		...
		handleGetData()
			...
			.catch((err) => dispatch(showError("Failed to fetch skipped tasks.")));
	}, [handleGetData]);
		...
		<div className="container">
			...
			<HistoryBar
				...
				fetchdata={(id, pageNumber, pageSize) =>
					getApplicationSkipReason(id, pageNumber, pageSize)
				}
			/>
