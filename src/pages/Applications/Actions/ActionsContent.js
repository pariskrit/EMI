...
import { getApplicationActions } from "services/History/application";

// Init styled components
const AC = ContentStyle();

const ActionsContent = ({ navigation, id, setIs404, state }) => {
	// Init state
	...
	const handleGetData = useCallback(async () => {
		...
		try {
			...
			dispatch(showError("Failed to fetch actions."));
			return false;
		}
	}, [id, setIs404]);
	...
	useEffect(() => {
		...
		handleGetData()
			...
			.catch((err) => dispatch(showError("Failed to fetch actions.")));
	}, [handleGetData]);
	...
	return (
		<div className="container">
			<TabTitle title={`${applicationName} Actions`} />
			{/* Start dialogs */}
			<HistoryBar
				id={id}
				...
				fetchdata={(id, pageNumber, pageSize) =>
					getApplicationActions(id, pageNumber, pageSize)
				}
			/>
			<AddActionDialog
				open={openAddDialog}
			...
			<DeleteDialog
				...
				deleteEndpoint="/api/ApplicationActions"
				deleteID={deleteID}
				...
				<>
					...
					<div className="detailsContainer">
						<DetailsPanel
							...
							description="Create and manage Task Actions"
						/>
