...
const EditDialog = ({ open, closeHandler, data, handleEditData }) => {
	...
	const handleSave = async () => {
		...
		try {
			...
			setIsUpdating(false);
			closeOverride();
			dispatch(showError("Failed to edit feedback classification."));
		}
	};
	const handleUpdateData = async () => {
		...
		try {
			...
			const result = await API.patch(
				`/api/ApplicationFeedbackClassifications/${data.id}`,
				[
					{
						op: "replace",
						path: "name",
			...
			>
				...
				<AED.ActionContainer>
					<DialogTitle id="alert-dialog-title">
						{<AED.HeaderText>Edit Feedback Classification</AED.HeaderText>}
					</DialogTitle>
					<AED.ButtonContainer>
						<AED.CancelButton
