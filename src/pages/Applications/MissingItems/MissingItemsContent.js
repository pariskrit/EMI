...
import { getApplicationMissingPartToolReason } from "services/History/application";
...
const MissingItemsContent = ({ navigation, id, setIs404, state }) => {
	...
	const handleGetData = useCallback(async () => {
		...
		try {
			...
			dispatch(showError("Failed to fetch missing parts or tools."));
			return false;
		}
	}, [id, setIs404]);
	...
	return (
		<div className="container">
			<TabTitle title={`${applicationName} Missing Part or Tool Reasons`} />
			{/* Start dialogs */}
			<HistoryBar
				id={id}
				...
				fetchdata={(id, pageNumber, pageSize) =>
					getApplicationMissingPartToolReason(id, pageNumber, pageSize)
				}
			/>
