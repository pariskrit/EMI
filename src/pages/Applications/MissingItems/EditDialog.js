...
const EditMissingItemDialog = ({
	...
	const handleUpdateData = async () => {
		// Attempting to update MI name
		try {
			let result = await API.patch(
				`/api/ApplicationMissingPartToolReasons/${data.id}`,
				[
					{
						op: "replace",
						path: "name",
	...
	const handleSave = async () => {
		...
		try {
			...
			setIsUpdating(false);
			closeOverride();
			dispatch(showError("Failed to edit missing part or tool."));
		}
	};
			...
			>
				...
				<AED.ActionContainer>
					<DialogTitle id="alert-dialog-title">
						{<AED.HeaderText>Edit Missing Part or Tool Reason</AED.HeaderText>}
					</DialogTitle>
					<AED.ButtonContainer>
						<AED.CancelButton
...
export default EditMissingItemDialog;
