	...
	return (
		<div>
			<AT.TableContainer component={Paper} elevation={0}>
				<Table aria-label="Defect Risk Ratings Table">
					<AT.TableHead>
						<TableRow>
							<TableCell
								onClick={() => {
							...
							<TableCell
								...
								className={cx(classes.publishRow, {
									[classes.selectedTableHeadRow]:
										currentTableSort[0] === "action",
									[classes.tableHeadRow]: currentTableSort[0] !== "action",
								})}
							>
								<AT.CellContainer>
									Action
					...
					<TableBody>
						{(searchQuery === "" ? data : searchedData).map((d, index) => (
							<TableRow key={d.id}>
								...
								<AT.DataCell>
									<AT.CellContainer>
										<AT.TableBodyText>{d.action}</AT.TableBodyText>
...
export default DefectRiskRatingsTable;
